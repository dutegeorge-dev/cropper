...
### Панель файлов и коллажи

Слева находится прокручиваемая панель-плитка со **всеми** фотографиями и видео из
папки — в том числе если файлов несколько тысяч. Клик по любой миниатюре сразу
открывает файл, а добавленные в будущий коллаж элементы подсвечиваются зелёным.
При открытии папки приложение сначала создаёт все уменьшенные миниатюры и
показывает прогресс. Первичная подготовка большой папки может занять некоторое
время, зато после её завершения прокрутка не запускает декодирование файлов и
остаётся плавной до выбора другой папки.

Чтобы создать коллаж:

1. Откройте нужное фото в панели слева и нажмите **«Добавить текущее фото»**.
...
1. Кликните в нужное место временной шкалы или перетащите ползунок. Позиция
   рассчитывается по всей длительности ролика, поэтому клик посередине открывает
   кадр примерно из середины видео.
